...
public sealed partial class SpreaderSystem : EntitySystem
{
    ...
    public void GetNeighbors(EntityUid uid, TransformComponent comp, ProtoId<EdgeSpreaderPrototype> prototype, out ValueList<(MapGridComponent, TileRef)> freeTiles, out ValueList<Vector2i> occupiedTiles, out ValueList<EntityUid> neighbors)
    {
        ...
        {
            var neighborPosition = tile + offset;
            if (!_map.TryGetTileRef(gridUid, grid, neighborPosition, out var tileRef) || tileRef.Tile.IsEmpty)
                continue;
            ...
            var directionEnumerator = _map.GetAnchoredEntitiesEnumerator(gridUid, grid, neighborPosition);
    ...
    /// <summary>
    /// Activates spreaders on the same and neighboring tiles.
    /// </summary>
