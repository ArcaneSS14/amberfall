...
{
    // We keep track of which tiles are blocking, and how much damage from explosions those explosion blockers can take.
    ...
    /// <seealso cref="UpdateBlockerMap(EntityUid, MapGridComponent, Vector2i)"/>
    public void UpdateBlockerMap(EntityUid gridId, Vector2i tile, MapGridComponent? grid = null)
    {
        if (Resolve(gridId, ref grid, false))
            UpdateBlockerMap(gridId, grid, tile);
    }
    ...
    /// </summary>
    /// <remarks>
    ///     Explosion propagation currently has no directional boundary cache.
    /// </remarks>
    public void UpdateBlockerMap(EntityUid gridId, MapGridComponent grid, Vector2i tile)
    {
        if (TryComp<ExplosionGridMapComponent>(gridId, out var cache) &&
            cache.Tiles.Remove(tile, out var tileData))
    ...
    private void GetExplosionTolerance(EntityUid uid, Span<FixedPoint2> explosionTolerance)
    {
        ...
        // that this will result in a non-explosion blocker.Entities that ONLY break via construction graph node changes
        ...
        // shouldn't be blocking structures anyways....
        ...
        foreach (var (id, index) in _explosionTypes)
        {
            ...
            foreach (var (type, value) in explosionType.DamagePerIntensity.DamageDict)
            {
                ...
                // But this shouldn't matter for explosion blockers.
    ...
    private void OnBlockerGridRemoved(EntityUid entity)
    {
        if (!TryComp(entity, out ExplosionGridMapComponent? blockerGrid))
            return;

        foreach (var tile in blockerGrid.Tiles.Values)
        {
            DecrementRefCount(tile.ToleranceCacheIndex);
        }

        RemComp<ExplosionGridMapComponent>(entity);
    }

    public override void ReloadMap()
    {
        var enumerator = EntityQueryEnumerator<ExplosionGridMapComponent, MapGridComponent>();
        while (enumerator.MoveNext(out var uid, out var blockerComp, out var mapGrid))
        {
            foreach (var pos in blockerComp.Tiles.Keys)
            {
                UpdateBlockerMap(uid, pos, mapGrid);
            }
        }
    }
